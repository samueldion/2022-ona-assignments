...
nodes <- nodes_temp %>% left_join(app_filter %>% distinct(examiner_id, .keep_all=T), 
                         by = c("examiner_id")) %>% 
  select(examiner_id, gender, race, examiner_art_unit3, examiner_name_first, examiner_name_last)
nodes %>% head()
...
### Check the biggest helper centrality
```{r}
V(graph)$degree
...
in general, men have a higher degree centrality.  The exact reasons could vary
but it is a clear observation.
